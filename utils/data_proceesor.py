...
class DataProcessor:
    ...
    def prepare_data(executor, df, selected_cols=None):
        ...
        if "사용자태그" in df_copy.columns: # 사용자태그 -> id
            ...
            processed_df = pd.DataFrame({"text": df_copy["combined_text"], "labels": df_copy["사용자태그"], "patent_id": df_copy["출원번호"]})
            processed_df["label"] = processed_df["labels"].map(executor.label2id)
        else:
            processed_df = pd.DataFrame({"text": df_copy["combined_text"], "patent_id": df_copy["출원번호"]})
    ...
    def create_balanced_datasetdict(df_chunked, tokenizer, test_size=0.2, random_state=25):
        ...
        for label in sorted(df_chunked['label'].unique()):
            label_data = df_chunked[df_chunked['label'] == label].sample(n=min_count, random_state=random_state).reset_index(drop=True)
        ...
        # DatasetDict 생성
        train_dataset_dict = {'text': train_df['text'].tolist(), 'label': train_df['label'].tolist()}
        test_dataset_dict = {'text': test_df['text'].tolist(), 'label': test_df['label'].tolist()}
